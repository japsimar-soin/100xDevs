...
//User login
function getUserIdx(arr, email, password) {
...
app.post("/login", (req, res) => {
	const userid = getUserIdx(arr, req.body.email, req.body.password);
	if (userid == -1) {
		res.status(401).send();
	} else {
		const details = {
			id: arr[userid].id,
			firstName: arr[userid].firstName,
			lastName: arr[userid].lastName,
		};
		res.status(200).json(details);
	}
});

//Fetch all user's names and ids from the server (Protected route)
